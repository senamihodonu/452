...
static Data get(Rep r, End e) { 
  ...
  if(e==Head){
    ...
    //free the head node
    free(curr);
    printf("first node from the head direction deleted!!\n");
    //decrement list size
    r->len=r->len-1;
  } else if(e==Tail){
    ...
    if(curr != 0){
      ...
      if(curr->np[Tail] == 0){
        ...
        r->len=r->len-1;
        free(curr);
        printf("single node in deq deleted!!\n");
      } else {
        Node temp = r->ht[Head];
        ...
        temp->np[Tail] = 0;
        free(last_node);
        printf("first node from the tail direction deleted!!\n");
        //decrement list size
        r->len=r->len-1;
      }
    }
...
///////////////////////////////////////////////////////////////////////////////////////////////////
extern Deq mtq_new(int size) {
  Rep r=(Rep)malloc(sizeof(*r));
  if (!r) ERROR("malloc() failed");
  pthread_mutex_init(&r->mtx, 0); //initialize lock
  pthread_cond_init(&r->cv,0); //initialize condition variable
  r->ht[Head]=0;
  r->ht[Tail]=0;
  r->len=0;
  r->size=size;
...
/*get: return the top data from an end, len--*/
static Data mtq_get(Rep r, End e) { 
...
/*Appends a new node onto an end (Head/Tail)*/
static void mtq_put(Rep r, End e, Data d) {
  pthread_mutex_lock(&r->mtx);
  printf("add to the head!!\n");
  //allocate node
  Node new_node = deq_new(r->size);
...
extern void mtq_head_put(Deq q, Data d) {        mtq_put(rep(q),Head,d); }
extern Data mtq_head_get(Deq q)         { return mtq_get(rep(q),Head); }

extern void mtq_tail_put(Deq q, Data d) {        mtq_put(rep(q),Tail,d); }
extern Data mtq_tail_get(Deq q)         { return mtq_get(rep(q),Tail); }